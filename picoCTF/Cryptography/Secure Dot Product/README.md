...
* `script.py`: Script utilizado para interactuar con el socket y derivar la clave.
...
### 3. Explotación
El script `script.py` automatiza la explotación utilizando la técnica de longitud unitaria ($L=1$):
...
**Payload utilizado (`script.py` - Fragmento del ataque de extensión):**
```python
...
![script.py reconectando hasta hallar el vector [220], extrayendo la clave AES al 100% y revelando la flag](./assets/CTF_2026-03-13_17-56-41.png)
*El ataque de extensión de longitud reconstruye los 32 bytes de la clave AES y descifra la bandera.*
